...
public async Task<IActionResult> UploadProductImage(IFormFile file, CancellationToken ct)
{
    ...
    // 3) Duplicate var mı? (clean/skipped olanlardan)
    var existing = await _db.UploadFiles.AsNoTracking()
        .Where(x => x.Sha256 == sha256 &&
                    (x.ScanStatus == "clean" || x.ScanStatus == "skipped") &&
                    !string.Equals(x.ScanStatus, "deleted", StringComparison.OrdinalIgnoreCase))
        .OrderByDescending(x => x.UploadedAt)
        .FirstOrDefaultAsync(ct);

    if (existing is not null)
    {
        // temp dosyayı sil ve mevcut url dön
        SafeDeleteFile(tempAbs);
        var existingUrl = "/" + existing.RelativePath.Replace("\\", "/");
        return Ok(new { imageUrl = existingUrl, sha256, duplicated = true });
    }

    // 4) Final klasöre taşı
    var fileName = $"{Guid.NewGuid():N}{ext}";
    var relPath = Path.Combine("uploads", "products", fileName).Replace("\\", "/");
    var finalAbs = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot",
    ...
    {
        // Move başarısızsa temp’i silip hata dön
        SafeDeleteFile(tempAbs);
        return StatusCode(500, new { message = "Could not store file." });
    }
    ...
    var url = "/" + relPath;
    return Ok(new { imageUrl = url, sha256, duplicated = false });
}
...
public async Task<IActionResult> DeleteProductImage([FromQuery] string imageUrl, CancellationToken ct)
{
    ...
    // /uploads/products/abc.jpg -> uploads/products/abc.jpg
    ...
    var abs = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", rel.Replace("/", Path.DirectorySeparatorChar.ToString()));
    ...
    // (opsiyonel) Ürünlerde kullanılıyor mu? Eğer kullanılıyorsa silme
    /* The line `var inUse = await _db.Products.AnyAsync(p => p.ImageUrl == ("/" + rel) || p.ImageUrl
    == rel, ct);` is checking if there are any products in the database that are currently using the
    image file specified by the `imageUrl` parameter. */
    var inUse = await _db.Products.AnyAsync(p => p.ImageUrl == ("/" + rel) || p.ImageUrl == rel, ct);
    if (inUse)
    ...
    CancellationToken ct = default)
{
    ...
    var items = await q
        ...
        .Take(pageSize)
        .Select(x => new Restaurant.Application.DTOs.Admin.UploadLogDto(
            x.Id,
            x.FileName,
            x.RelativePath,
            x.ContentType,
    ...
    return Ok(new Restaurant.Application.DTOs.PagedResult<Restaurant.Application.DTOs.Admin.UploadLogDto>(
